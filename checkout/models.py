...
class Order(models.Model):
    ...
    # sets the timefield to the current one
    ...
    def _generate_order_number(self):
        ...
        '''
        return uuid.uuid4().hex.upper()  # generates random string of 32 chars for order number
    ...
    def update_total(self):
        ...
        '''
        # 'or 0' is added to prevent an error if all lineitems are manually deleted, setting the order value to 0 instead of None
...
class OrderLineItem(models.Model):
    ...
    product_size = models.CharField(max_length=2, null=True,
                                    blank=True)  # XS, S, M, L, XL; null and blank allows for items with not sizes
    quantity = models.IntegerField(null=False, blank=False, default=0)
    lineitem_total = models.DecimalField(max_digits=6, decimal_places=2,
                                         null=False, blank=False,
                                         editable=False)  # not editable as automatically calculated when the item is saved
